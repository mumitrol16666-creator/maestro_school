  ...
  return Promise.all(students.map(async (student): Promise<WhatsappHomeworkMessageDraft> => {
    ...
    return {
      ...
      note: env.OPENAI_API_KEY
        ? "Не удалось обновить формулировку, поэтому подготовлен стандартный черновик."
        : "Сейчас подготовлен стандартный черновик. После подключения AI формулировки будут разнообразнее.",
    };
  }));
}
