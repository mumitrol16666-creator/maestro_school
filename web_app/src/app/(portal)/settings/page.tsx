...
import { BookOpen, GraduationCap, Mail, Phone, Star, UserRound } from "lucide-react";
      ...
      <div className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
        ...
        <section className="space-y-5">
          ...
          <div className="rounded-[30px] border border-stone-200 bg-paper p-6 shadow-soft sm:p-8">
            ...
            <div className="mt-5 space-y-3">{courses.length ? courses.map((course) => <div key={course.id} className="flex items-center gap-4 rounded-2xl bg-stone-50 p-4"><span className="grid h-10 w-10 place-items-center rounded-xl bg-white text-gold"><BookOpen size={17} /></span><div><p className="font-bold">{course.title}</p><p className="mt-1 text-xs text-stone-400">{course.direction.title}</p></div></div>) : <p className="text-sm text-stone-500">Вы еще не начали ни одного курса.</p>}</div>
          </div>
        </section>
      </div>
    </>
