        ...
        {plan.items.map((item) => {
          ...
          return (
            <div key={item.id} className="flex items-start gap-3 py-4">
              ...
              <span className="min-w-0 flex-1 break-words text-sm font-bold leading-6 text-stone-800">{item.title}</span>
              <span className={`shrink-0 text-xs font-bold ${status.className}`}>{status.label}</span>
            </div>
          );
        })}
      </div>
