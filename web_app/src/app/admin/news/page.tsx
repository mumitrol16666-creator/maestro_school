...
export default function NewsAdminPage() {
  ...
  const [form, setForm] = useState(empty); const [baseline, setBaseline] = useState(empty); const [saving, setSaving] = useState(false); const [saved, setSaved] = useState(false);
  ...
  async function save(event: FormEvent) { event.preventDefault(); setSaving(true); try { if (editing) await cmsApi.updateNews(editing.id, form); else await cmsApi.createNews(form); setEditing(null); setForm(empty); setBaseline(empty); setSaved(true); await resource.reload(); } finally { setSaving(false); } }
  function edit(item: CmsNews) { if (dirty && !window.confirm("Есть несохранённые изменения. Продолжить и потерять их?")) return; const value = { title: item.title, content: item.content }; setEditing(item); setForm(value); setBaseline(value); setSaved(false); }
