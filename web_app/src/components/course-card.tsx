    ...
    >
      ...
      <div className="p-6">
        ...
        <div className="mt-5 flex items-center justify-between text-xs font-semibold text-stone-500">
          ...
          <span>{enrolled ? `${course.progress}% пройдено` : "Можно начать сейчас"}</span>
        </div>
        ...
        <div className="mt-5 flex items-center gap-2 text-sm font-bold text-ink">
          {enrolled ? <Play size={15} fill="currentColor" /> : <BookOpen size={15} />}
          {enrolled ? "Продолжить обучение" : "Посмотреть курс"}
        </div>
      </div>
    </Link>
  );
