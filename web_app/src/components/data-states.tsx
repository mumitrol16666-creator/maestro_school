...
export function EmptyState({ title, description }: { title: string; description: string }) {
  return (
    <div className="grid min-h-[360px] place-items-center rounded-[30px] border border-stone-200 bg-paper p-8 text-center shadow-soft">
      <div className="max-w-md"><Inbox className="mx-auto text-gold" /><h2 className="font-display mt-4 text-3xl">{title}</h2><p className="mt-3 text-sm leading-6 text-stone-500">{description}</p></div>
    </div>
  );
}
